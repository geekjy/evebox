...
resolve_asset_url() {
  ...
  if [ -n "$RELEASE_TAG" ]; then
    github_release_asset_url "https://api.github.com/repos/${REPO}/releases/tags/${RELEASE_TAG}" "$regex"
    return
  fi
...
github_latest_asset_url() {
  ...
  local json errf errtxt
  errf="$(mktemp)"
  if ! json="$(github_api_get "https://api.github.com/repos/${REPO}/releases/latest" 2>"$errf")"; then
    errtxt="$(tr '[:upper:]' '[:lower:]' < "$errf" || true)"
    if [[ "$errtxt" == *"403"* || "$errtxt" == *"rate limit"* ]]; then
      die "GitHub API returned 403 (likely rate limit). Export GITHUB_TOKEN and retry."
    fi
